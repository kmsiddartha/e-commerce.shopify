...
const AddressForm = ({checkoutToken,next}) => {
  ...
  const [shippingOption, setShippingOption] = useState('');
    ...
    const fetchShippingCountries=async(checkoutTokenId)=>{
        ...
        setShippingCountries(countries);
        setShippingCountry(Object.entries(countries)[0]);
    };
            ...
            <FormProvider {...methods}>
                <form onSubmit={methods.handleSubmit((data)=>next({...data,shippingCountry,shippingOption,shippingSubdivision}))}>
                    ...
                    <FormInput required name="email" label="Email" />
                     <FormInput required name="city" label="City" gutterBottom />
                     <FormInput required name="zip" label="Zip / Postal code"  gutterBottom/>
            ...
            <Grid item xs={12} sm={6}>
              ...
              <Select value={shippingCountry} fullWidth onChange={(e) => setShippingCountry(e.target.value)}>
                {Object.entries(shippingCountries).map(([code, name]) => ({ id: code, label: name })).map((country) => (
                  <MenuItem name='Ship' key={country.id} value={country.id}>
                    {country.label}
                  </MenuItem>
                ))}
              </Select>
